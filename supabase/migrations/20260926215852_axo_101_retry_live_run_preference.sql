-- ============================================================================
-- AXO-101 — failed-paper retry is one transaction and one live extraction run
-- ============================================================================
-- The existing submit_paper(p_paper_id => ...) path already reuses stored page
-- rows without erasing omitted keys. Retry therefore remains the same processor.
-- This revision adds a paper-scoped advisory lock around run selection/creation,
-- chooses the latest run (rather than any older non-failed run), and resets only
-- page-stage state when a genuinely fresh run is created.
-- ============================================================================
...
revoke all on function public.submit_paper(
  uuid, public.paper_type, public.paper_tier, date, text,
  jsonb, uuid, numeric, numeric, text, uuid
) from public, anon;
grant execute on function public.submit_paper(
  uuid, public.paper_type, public.paper_tier, date, text,
  jsonb, uuid, numeric, numeric, text, uuid
) to authenticated, service_role;

comment on function public.submit_paper(
  uuid, public.paper_type, public.paper_tier, date, text,
  jsonb, uuid, numeric, numeric, text, uuid
) is
  'AXO-101. Idempotent paper submit/resubmit. Paper-scoped advisory locking plus reuse of any existing non-failed/non-rejected run guarantees one live extraction run; stored page keys survive, and page-stage state resets only when a fresh run is created.';
